...
export default function ProjectMainDetail({ index, elem, progressBar, totalTask, completeTask }: any) {
    const { setAddMemberModal } = useProjectUi1()
    const { getSingleProject } = useProject()
    return (
        <div key={elem.id} className="min-h-14 grid grid-cols-[0.5fr_2fr_3fr_1.2fr_1.3fr_1.3fr_1.6fr_1fr_2fr_1fr] items-center border-b border-[#252938] px-4 text-sm text-gray-300">
            ...
            <div className="flex items-center gap-2 pr-4">
                <div className="h-1.5 w-20 overflow-hidden rounded-full bg-[#252938]">
                    <div style={{ width: `${progressBar}` }} className={`h-full w-full rounded-full bg-blue-500 transition-all`}></div>
                </div>
            ...
            <div className="flex items-center justify-center gap-2">
                <button className="rounded-md p-1.5 text-gray-400 transition hover:bg-blue-500/10 hover:text-blue-400">
                    <RiEdit2Fill className="h-4 w-4" />
                </button>

                <button className="rounded-md p-1.5 text-gray-400 transition hover:bg-red-500/10 hover:text-red-400">
